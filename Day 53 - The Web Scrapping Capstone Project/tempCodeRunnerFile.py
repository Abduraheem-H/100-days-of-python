...
listings = soup.select("#grid-search-results ul.photo-cards > li")
print(f"Total listings found: {len(listings)}")

for item in listings:
    address = item.find("address").getText(strip=True)
    price = item.find("span").getText(strip=True).split("/")
    property_link = item.find("a")
    print(f"Address: {address}")
    print(f"Price: {price[0]}")
    print(f"Link: {property_link['href']}")
    break
